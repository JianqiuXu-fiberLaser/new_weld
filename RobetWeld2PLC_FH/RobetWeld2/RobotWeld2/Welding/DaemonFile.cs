...
namespace RobotWeld2.Welding
{
    ...
    public class DaemonFile
    {
        ...
        public void GetLaserParameter(InputViewModel ivm)
        {
            ...
            if (cp != null && cp.Count > 1)
            {
                ivm.LaserPower = cp[0].LaserPointPower;
                ivm.WeldSpeed = cp[0].WeldSpeed;
                ivm.WobbleSpeed = cp[0].Wobble;
                ivm.LeapSpeed = cp[0].Leap;
            }
            else
            {
                ivm.LaserPower = 0;
        ...
        public void AddPoint(int x, int y, int z)
        {
            ...
            else if (ExaminePoints())
            {
                if (_points != null)
                {
                    ...
                    cdt.CompleteTrace(_viewModel.TraceDir, _viewModel.TraceInfo, _viewModel.PtIndex, vc, ref cp);
                }
            }
        ...
        internal void MatchPoint()
        {
            if (_points != null)
            {
                var amp = new AutoMatchPoint();
                amp.PutPointList(_points);
                amp.CompleteTrace(_viewModel.TraceDir);
                DisplayPointList(_points);
            }
        }
