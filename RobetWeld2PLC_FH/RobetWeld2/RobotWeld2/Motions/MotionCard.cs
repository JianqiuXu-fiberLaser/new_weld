...
namespace RobotWeld2.Motions
{
    ...
    public class MotionCard
    {
        ...
        public void ReadCoordinate()
        {
            ...
            var stxyz = new SetXYZDelegate(tkc.SetXYZ);
        ...
        {
            var addpt = new AddPointDelegate(tkc.SetXYZ);
            int[] pos = new int[3];
            for (short i = 0; i < 3; i++)
            {
                if (axisHandle != null)
            ...
            double moveSpeed, double synAcc, int addr, int mode = 0)
        {
            if (mode == 0)
            {
                var weldProcess = new WeldProcess();
    ...
    //
    public delegate void AddPointDelegate();
    public delegate void SetXYZDelegate();
}
