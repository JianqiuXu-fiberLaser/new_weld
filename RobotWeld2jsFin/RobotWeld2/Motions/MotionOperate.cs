...
namespace RobotWeld2.Motions
{
    ...
    public class MotionOperate
    {
        ...
        {
            LaserDrives.SetAnalog(0, wobble);
        }
        ...
        /// <summary>
        /// Adjust laser power and switch on
        /// </summary>
        /// <param name="arg"></param>
        /// <param name="power"></param>
        public void AdjustLaserPower(in int power)
        {
            if (maxPower != 0)
            {
                EnableSW();
        ...
        public void MoveLine(List<int[]> pts, List<double> speed, int StartSegNum)
        {
            if (pts.Count == 1)
            {
                lock (_lock)
                {
                    ...
                    double sp = speed[0];
                    if (sp == 0) sp = 25;
                    _mb.LineSingleMove(xyz, sp, startSeg: StartSegNum);
                }
            }
            else
            {
