...
namespace RobotWeld2.Welding
{
    ...
    public class IntersectTrace
    {
        ...
        private void PrepareData()
        {
            ...
            AssembleSeam(in points, ds, out List<int[]> ptlist);

            if (points.GetCount() > 2)
            {
                CorrectSeam(in points, ptlist);
            }

            TransferToWms(ptlist, in points);
        }
        ...
        //
        private void AssembleSeam(in PointListData pts, List<double> ds, out List<int[]> ptlist)
        {
            int direction;
            double xstep, zstep;
            if (_InstalStyle == 0)
            ...
            else if (_InstalStyle == 1)
            {
                direction = -1;
                xstep = 0.0;
                zstep = MotionOperate.Zmillimeter;
            }
            else if (_InstalStyle == 2)
            {
                direction = 1;
        ...
        //
        // If there are more than two points, the trace is corrected using these points.
        // In this version, the points is constricted to the lowest points only.
        //
        private void CorrectSeam(in PointListData pts, List<int[]> ptlist)
        {
            if (pts.GetCount() >= 4)
            {
        ...
        private void TwoPointCorrect(in PointListData pts, List<int[]> ptlist)
        {
            ...
            // The z step
            if (_InstalStyle == 0 || _InstalStyle == 1)
            {
            ...
            else
            {
                ...
                double coefx0 = delx0 / calx;
                double coefx1 = delx1 / calx;
