...
namespace RobotWeld2
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void CommandExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            {
                mainModel?.WeldTrace(Tracetype.VANE_WHEEL);
            }

            //
            // Run Intersect welding Process
            //
            if (e.Command == MainWindowViewModel.RunIntersectCommand)
            {
                mainModel?.WeldTrace(Tracetype.INTERSECT);
            }

            if (e.Command == MainWindowViewModel.RunTopTraceCommand)
            {
                mainModel?.WeldTrace(Tracetype.TOP_TRACE);
            }

            if (e.Command == MainWindowViewModel.RunStageTraceCommand)
            {
                mainModel?.WeldTrace(Tracetype.STAGE_TRACE);
            }

            if (e.Command == MainWindowViewModel.RunSpiralCommand)
            {
                mainModel?.WeldTrace(Tracetype.SPIRAL);
            }

            if (e.Command == MainWindowViewModel.RunFreeCurveCommand)
            {
                mainModel?.WeldTrace(Tracetype.FREE_TRACE);
            }
            ...
            {
                mainModel?.ConfigTrace(Tracetype.TOP_TRACE);
            }
