...
namespace RobotWeld2.Motions
{
    ...
    public class MotionOperate
    {
        ...
        public static double OneCycle = 16000.0;
        public static double OneMillimeter = 1000.0;
        ...
        public void InitialLaser(int LaserMode)
        {
            if (LaserMode == 0)
            {
                _laser.InitialLaser(0, 0, 0);
            }
            else if (LaserMode == 1)
            {
                if (_laserWeldParameter != null && _laserWeldParameter.DutyCycle >= 0 && _laserWeldParameter.DutyCycle <= 100)
                {
                    _laser.InitialLaser(1, _laserWeldParameter.Frequency, _laserWeldParameter.DutyCycle);
                }
            }
        }
        ...
        public void SetupLaserParameter(in LaserWeldParameter parameter, int MaxPower)
        {
            if (MaxPower == 0)
            {
            ...
            else
            {
                ...
                _maxPower = MaxPower;
                _laserWeldParameter.LaserPower = parameter.LaserPower;
                _laserWeldParameter.Frequency = parameter.Frequency;
                _laserWeldParameter.PulseWidth = parameter.PulseWidth;
                _laserWeldParameter.DutyCycle = parameter.DutyCycle;
                _laserWeldParameter.LaserRise = parameter.LaserRise;
                _laserWeldParameter.LaserFall = parameter.LaserFall;
                _laserWeldParameter.LaserHoldTime = parameter.LaserHoldTime;
                _laserWeldParameter.AirIn = parameter.AirIn;
                _laserWeldParameter.AirOut = parameter.AirOut;
                _laserWeldParameter.WireTime = parameter.WireTime;
                PowerSubdivision();
            }
        }
        ...
        {
            if (_laserWeldParameter == null) return;

            int LaserPower = _laserWeldParameter.LaserPower;
        ...
        private void K3Changed()
        {
            ...
            tkn?.SetXYZ(pos[0], pos[1], pos[2], pos[3]);
        }
        ...
        public void RunResetAxis()
        {
            ...
            wthread.Start(); Thread rthread = new(_mb.ResetAaxis)
            {
                IsBackground = true,
            };
            rthread.Start();
        ...
        public bool RunHandLeapRotate(int[] pts, double moveSpeed, double synAcc = 0.5)
        {
            //_threadFlag = false;
            Thread.Sleep(5);
            ...
            HandLeapFrog hlf = new(this);
            hlf.SetParameter(pts, moveSpeed, synAcc);
            wthread = new Thread(hlf.HandLeapRotate)
            {
                Name = nameof(RunLeap),
                IsBackground = true,
            };
        ...
        class HandLeapFrog : MotionOperate
        {
            ...
            public void HandLeapRotate()
            {
                if (pts != null)
                {
                    _mutex.WaitOne(3000);
                    _mb.LeapRotateMove(pts, 0.3 * moveSpeed, synAcc);
                    _mutex.ReleaseMutex();
                }
                else
                {
        ...
        class FlyWeldTrace : MotionOperate
        {
                ...
                int[] pts, double moveSpeed, double synAcc = 0.5)
            {
                ...
                {
                    this.maxPower = lp.MaxPower;
                    powerPercent = (double)this.laserPower / (double)maxPower;
                }
            }
        ...
        //
        // THREAD: rotate the A-axis, meanwhile the XYZ axis moveing accordingly
        //
        public bool RunRotateWeld(List<int[]> ptsList, List<int> Apts, double moveSpeed,
            double WeldSpeed, List<double> CofWeld, double synAcc = 0.5)
        {
            ...
            RotateWeldTrace rwt = new(this);
            rwt.SetParameter(ptsList, Apts, moveSpeed, WeldSpeed, CofWeld, synAcc);

            wthread = new Thread(rwt.RotateWeldThread)
            {
                Name = nameof(RunRotateWeld),
                IsBackground = true,
            };
            wthread.Start();
        ...
        class RotateWeldTrace : MotionOperate
        {
            private List<int[]>? ptsList;
            private List<int>? Apts;
            ...
            public RotateWeldTrace(MotionOperate mo)
            {
                this._mb = mo._mb;
                this._laser = mo._laser;
                this.vm = mo.vm;
                this.ist = mo.ist;
                this.methodName = mo.methodName;
                this.powerPercent = mo.SubPowerPercent[4];
            }

            public void SetParameter(List<int[]> ptsList, List<int> Apts, double moveSpeed,
                double WeldSpeed, List<double> CofWeld, double synAcc = 0.5)
            {
                this.ptsList = ptsList;
                this.Apts = Apts;
                this.CofWeld = CofWeld;
                this.WeldSpeed = WeldSpeed;
                this.moveSpeed = moveSpeed;
                this.synAcc = synAcc;
            }

            public void RotateWeldThread()
            {
                if (Apts != null && ptsList != null && CofWeld != null && ptsList.Count > 0)
                {
                    var pts = ptsList.GetRange(1, ptsList.Count - 1);

                    _mutex.WaitOne(3000);
                    _mb.FlyMove(ptsList[0], moveSpeed, synAcc);
                    OpenAir();
                    _mb.FlyRotateMove(pts[0], Apts[0], 0.5 * moveSpeed, synAcc);

                    _laser.LaserOn(powerPercent);

                    _mb.RotateMove(pts, Apts, WeldSpeed, CofWeld, synAcc);
                    _mutex.ReleaseMutex();
        ...
        protected void FinAction(string iAct)
        {
            ...
            else if (this.ist != null && methodName == nameof(IntersectTrace))
            {
                act = new(ist.FinAction);
                act(iAct);
            }
        }

        private IntersectTrace? ist;
        ...
        public void SetActionMethod(IntersectTrace ist)
        {
            this.ist = ist;
            methodName = nameof(IntersectTrace);
        }
